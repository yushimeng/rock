...
func (l *Layer) ServeUDP(c net.PacketConn) error {
	...
	transport := NewUDPTransport(c.LocalAddr().String(), sip.NewParser())
...
func (l *Layer) ServeTCP(c net.Listener) error {
	...
	transport := NewTCPTransport(c.Addr().String(), sip.NewParser())
...
func (l *Layer) ServeWS(c net.Listener) error {
	...
	transport := NewWSTransport(c.Addr().String(), sip.NewParser())
...
func (l *Layer) ServeTLS(c net.Listener, conf *tls.Config) error {
	...
	transport := NewTLSTransport(c.Addr().String(), sip.NewParser(), conf)
